...
import {
  ...
  type EmojiMenuStaticConfig,
} from './config.js';
import { EmojiMenu } from './emoji-menu-popover.js';
import { filterEnabledEmojiItems } from './utils.js';

export type AffineEmojiMenuContext = EmojiMenuContext;

let globalEmojiAbortController = new AbortController();

function closeEmojiMenu() {
  globalEmojiAbortController.abort();
}
...
const showEmojiMenu = debounce(
  ({
    ...
    globalEmojiAbortController = abortController;
    ...
    const updatePosition = throttle(() => {
      ...
      assertExists(
        emojiMenuElement,
        'The emoji menu node should be rendered even if no position'
      );
...
export class AffineEmojiMenuWidget extends WidgetComponent {
  ...
  private _handleInput = (
    ...
    inlineRangeApplyCallback(() => {
      ...
      const config: EmojiMenuStaticConfig = {
        ...this.config,
        items: filterEnabledEmojiItems(this.config.items, {
          model,
          rootComponent,
        }),
      };
  ...
  private _onKeyDown = (ctx: UIEventStateContext) => {
    ...
    if (key === undefined || key === 'Process' || isComposing) return;
